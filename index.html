...
.page{max-width:100%;margin:0 auto;padding:0 16px 100px;position:relative;z-index:1}
...
.hdr{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:20px;padding:10px 16px;max-width:100%;margin:0 auto;}
...
.metric{background:var(--card);border:1px solid var(--rim);border-radius:12px;padding:12px 16px;position:relative;overflow:hidden;box-shadow:var(--sh);transition:transform .18s,box-shadow .18s}
.metric:hover{transform:translateY(-2px);box-shadow:var(--sh2)}
.metric-icon{font-size:16px;margin-bottom:4px;line-height:1}
.metric-val{font-size:26px;font-weight:800;letter-spacing:-1px;line-height:1;font-variant-numeric:tabular-nums}
.metric-label{font-size:10px;font-family:var(--f-mono);color:var(--t2);text-transform:uppercase;letter-spacing:.8px;margin-top:4px}
...
.vw-cols table th{padding:6px 6px;font-size:8px;letter-spacing:.4px}
.vw-cols table td{padding:5px 6px;font-size:9px}
.vw-cols table{table-layout:fixed;width:100%}
.vw-cols>div{min-width:0}
.vw-cols .tbl-shell{overflow:hidden}
...
@media(max-width:600px){
  ...
  .hdr{grid-template-columns:1fr auto;grid-template-rows:auto auto;gap:6px;padding:8px 10px}
  .hdr-brand{grid-column:1;display:flex;align-items:center;gap:8px}
  .hdr-brand img{height:24px!important}
  .hdr-brand>div div:first-child{font-size:12px}
  .hdr-brand>div div:last-child{display:none}
  .hdr-actions{grid-column:2;grid-row:1;display:flex;align-items:center;gap:3px}
  .hdr-actions .sync-time{display:none}
  .hdr-actions .icon-btn{width:28px;height:28px;font-size:12px}
  .hdr-actions .refresh-ring{width:28px;height:28px}
  .hdr-actions .refresh-btn{padding:5px 10px;font-size:11px}
  .hdr-center{grid-column:1/-1;display:flex;justify-content:center;padding-bottom:4px}
  .hdr-center>div{gap:3px}
  .hdr-center>div>div:first-child button{padding:4px 10px;font-size:10px}
  .hdr-center>div .live-badge{font-size:9px;padding:2px 8px}
  .metric-strip{grid-template-columns:repeat(2,1fr);gap:8px}
  .metric{padding:12px 12px}
  .metric-val{font-size:24px}
  .hour-grid{grid-template-columns:repeat(2,1fr);gap:8px}
  .hcard-n{font-size:24px}
  .hcard{padding:12px 10px}
  .filter-row{grid-template-columns:repeat(2,1fr)}
  .chart-pair{grid-template-columns:1fr}
  .drw{width:100vw}
  .drw-stats{grid-template-columns:repeat(2,1fr)}
  /* VIN wise — stack 3 cols vertically on mobile */
  .vw-cols{grid-template-columns:1fr!important;gap:20px}
  /* VIN wise summary cards — 1 col on mobile */
  #vw-screen>div:nth-child(2){grid-template-columns:1fr!important}
  .tbl-scroll{max-height:280px}
}
@media(min-width:601px) and (max-width:1024px){
  /* Tablet — stack VIN wise to 1 col */
  .vw-cols{grid-template-columns:1fr!important;gap:16px}
  #vw-screen>div:nth-child(2){grid-template-columns:1fr 1fr!important}
}
</style>
</head>
<body>
  ...
  <div class="hdr-actions" style="gap:4px">
    ...
    <div id="main-refresh-group" style="display:flex;align-items:center;gap:0">
      <div class="refresh-ring" title="Auto-refresh 3min">
        ...
        <div class="refresh-ring-label" id="ring-label">15:00</div>
      </div>
...
<div id="dash" style="display:none">
  ...
  <div class="sec">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-bottom:8px" id="hcards"></div>
  </div>
  ...
  <div class="sec">
    ...
    <div class="chart-pair">
      ...
      <div class="chart-panel">
        <div class="chart-panel-title">By QC User</div>
        ...
        <div style="overflow-x:auto;overflow-y:auto;max-height:175px">
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            <thead style="position:sticky;top:0;z-index:2"><tr>
              <th style="padding:7px 10px;text-align:left;font-family:var(--f-mono);font-size:9px;color:var(--t2);font-weight:500;letter-spacing:.6px;background:var(--card2);border-bottom:1px solid var(--rim2)">QC USER</th>
              <th style="padding:7px 10px;text-align:left;font-family:var(--f-mono);font-size:9px;color:var(--t2);font-weight:500;letter-spacing:.6px;background:var(--card2);border-bottom:1px solid var(--rim2)">ENTERPRISE</th>
              <th style="padding:7px 10px;text-align:center;font-family:var(--f-mono);font-size:9px;color:var(--green);font-weight:500;letter-spacing:.6px;background:var(--card2);border-bottom:1px solid var(--rim2)">BELOW 6H</th>
              <th style="padding:7px 10px;text-align:center;font-family:var(--f-mono);font-size:9px;color:var(--red);font-weight:500;letter-spacing:.6px;background:var(--card2);border-bottom:1px solid var(--rim2)">ABOVE 6H</th>
              <th style="padding:7px 10px;text-align:center;font-family:var(--f-mono);font-size:9px;color:var(--purple);font-weight:500;letter-spacing:.6px;background:var(--card2);border-bottom:1px solid var(--rim2)">REPEAT</th>
            </tr></thead>
            <tbody id="ch-crm"></tbody>
          </table>
        </div>
      </div>
    </div>
  ...
  <!-- Processing -->
  <div class="sec" id="processing-sec">
    <div class="sec-hd">
      <div class="sec-label"><span class="sec-title">Processing</span><span class="tag tag-teal" id="proc-tag">AI PENDING</span></div>
      <button onclick="dlProcCsv()" style="display:inline-flex;align-items:center;gap:5px;padding:5px 12px;border-radius:7px;border:1px solid rgba(45,212,207,.3);background:rgba(45,212,207,.06);color:var(--teal);font-size:11px;font-family:var(--f-mono);font-weight:500;cursor:pointer">
        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        CSV
      </button>
    </div>
    <div class="tbl-shell"><div class="tbl-scroll"><table>
      <thead><tr>
        <th style="min-width:130px">SKU ID</th>
        <th style="min-width:160px">VIN</th>
        <th style="min-width:90px">Ent ID</th>
        <th style="min-width:180px">Enterprise</th>
        <th style="min-width:100px">Input Type</th>
        <th style="min-width:120px">CRM Status</th>
        <th>Age</th>
      </tr></thead>
...
<div id="vw-screen" style="display:none">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;flex-wrap:wrap;gap:12px">
    ...
    <div style="display:flex;align-items:center;gap:8px">
      <span style="font-size:10px;font-family:var(--f-mono);color:var(--t3)">Category:</span>
      <div class="dl-dropdown" id="vw-cat-dd">
        <button class="dl-trigger" onclick="toggleDl('vw-cat-dd')" id="vw-cat-btn" style="font-size:10px;padding:4px 10px;min-width:80px">
          All
          <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>
        </button>
        <div class="dl-menu" id="vw-cat-menu" style="min-width:120px"></div>
      </div>
    </div>
  </div>
  <!-- 1. Segment Status — top -->
  <div style="margin-bottom:10px">
    <div style="font-size:13px;font-weight:700;color:var(--t1);margin-bottom:8px;display:flex;align-items:center;gap:8px">
      Segment Status
      <span style="font-size:10px;font-family:var(--f-mono);color:var(--t3);font-weight:400">by customer segment</span>
    </div>
    <div class="tbl-shell"><div class="tbl-scroll" style="max-height:175px">
      <table>
        <thead><tr>
          <th style="min-width:120px">Segment</th>
          <th style="text-align:center;color:var(--green)">Within SLA</th>
          <th style="text-align:center;color:var(--red)">Over SLA</th>
          <th style="text-align:center;color:var(--purple)">Repeat SKU</th>
        </tr></thead>
        <tbody id="vw-seg-tbody"></tbody>
      </table>
    </div></div>
  </div>
  <!-- 2. Summary cards — compact -->
  <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) minmax(0,1fr);gap:8px;margin-bottom:10px">
    <div style="background:rgba(0,200,150,.08);border:1px solid rgba(0,200,150,.25);border-radius:8px;padding:6px 14px;display:flex;align-items:center;gap:12px">
      <div>
        <div style="font-size:8px;font-family:var(--f-mono);color:var(--green);letter-spacing:.8px">BELOW 6H · WITHIN SLA</div>
        <div style="font-size:18px;font-weight:800;color:var(--green);letter-spacing:-1px;line-height:1.2" id="vw-below-count">0</div>
      </div>
      <div style="font-size:10px;color:var(--green);opacity:.7">VINs on track</div>
    </div>
    <div style="background:rgba(240,81,110,.08);border:1px solid rgba(240,81,110,.25);border-radius:8px;padding:6px 14px;display:flex;align-items:center;gap:12px">
      <div>
        <div style="font-size:8px;font-family:var(--f-mono);color:var(--red);letter-spacing:.8px">ABOVE 6H · SLA BREACHED</div>
        <div style="font-size:18px;font-weight:800;color:var(--red);letter-spacing:-1px;line-height:1.2" id="vw-above-count">0</div>
      </div>
      <div style="font-size:10px;color:var(--red);opacity:.7">VINs breached</div>
    </div>
    <div style="background:rgba(180,143,255,.08);border:1px solid rgba(180,143,255,.25);border-radius:8px;padding:6px 14px;display:flex;align-items:center;gap:12px">
      <div>
        <div style="font-size:8px;font-family:var(--f-mono);color:var(--purple);letter-spacing:.8px">REPEAT VINs</div>
        <div style="font-size:18px;font-weight:800;color:var(--purple);letter-spacing:-1px;line-height:1.2" id="vw-repeat-count">0</div>
      </div>
      <div style="font-size:10px;color:var(--purple);opacity:.7">first_qc_done set</div>
    </div>
  </div>
  <!-- 3-column tables — full width equally divided -->
  <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) minmax(0,1fr);gap:12px" class="vw-cols">
    <!-- Below 6h -->
    <div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:6px">
        <div style="font-size:13px;font-weight:700;color:var(--green);display:flex;align-items:center;gap:6px">
        ...
        <button onclick="dlVwData('below','csv')" style="display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:6px;border:1px solid rgba(0,200,150,.3);background:rgba(0,200,150,.06);color:var(--green);font-size:10px;font-family:var(--f-mono);font-weight:600;cursor:pointer" onmouseover="this.style.background='rgba(0,200,150,.15)'" onmouseout="this.style.background='rgba(0,200,150,.06)'">
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>CSV
        </button>
      </div>
      <div class="tbl-shell"><div class="tbl-scroll">
        <table>
          <thead><tr>
            <th style="width:12%">SKU ID</th><th style="width:18%">VIN</th><th style="width:12%">Ent ID</th><th style="width:22%">Enterprise</th>
            <th style="width:10%" onclick="_vwBelowSort=_vwBelowSort==='desc'?'asc':'desc';renderVinWise()" style="cursor:pointer;user-select:none">AGE <span id="vw-below-sort-icon">↓</span></th>
    ...
    <div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:6px">
        ...
        <button onclick="dlVwData('above','csv')" style="display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:6px;border:1px solid rgba(240,81,110,.3);background:rgba(240,81,110,.06);color:var(--red);font-size:10px;font-family:var(--f-mono);font-weight:600;cursor:pointer" onmouseover="this.style.background='rgba(240,81,110,.15)'" onmouseout="this.style.background='rgba(240,81,110,.06)'">
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>CSV
        </button>
      </div>
      <div class="tbl-shell" style="border-color:rgba(240,81,110,.3)"><div class="tbl-scroll">
        <table>
          <thead style="background:var(--card2)"><tr style="background:var(--card2);border-bottom:1px solid rgba(240,81,110,.3)">
            <th style="color:var(--red)">SKU ID</th><th style="color:var(--red)">VIN</th>
            <th style="color:var(--red)">Ent ID</th><th style="color:var(--red)">Enterprise</th>
            <th onclick="_vwAboveSort=_vwAboveSort==='desc'?'asc':'desc';renderVinWise()" style="cursor:pointer;user-select:none;color:var(--red)">AGE <span id="vw-above-sort-icon">↓</span></th>
    ...
    <div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:6px">
        <div style="font-size:13px;font-weight:700;color:var(--purple);display:flex;align-items:center;gap:6px">
          <span style="width:8px;height:8px;border-radius:50%;background:var(--purple);display:inline-block;flex-shrink:0"></span>
          Repeat VIN
          <span style="font-size:10px;font-family:var(--f-mono);color:var(--t3);font-weight:400">first_qc_done set</span>
        </div>
        <button onclick="dlVwData('repeat','csv')" style="display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:6px;border:1px solid rgba(180,143,255,.3);background:rgba(180,143,255,.06);color:var(--purple);font-size:10px;font-family:var(--f-mono);font-weight:600;cursor:pointer" onmouseover="this.style.background='rgba(180,143,255,.15)'" onmouseout="this.style.background='rgba(180,143,255,.06)'">
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>CSV
        </button>
      </div>
      <div class="tbl-shell" style="border-color:rgba(180,143,255,.3)"><div class="tbl-scroll">
        <table>
          <thead style="background:var(--card2)"><tr style="background:var(--card2);border-bottom:1px solid rgba(180,143,255,.3)">
            <th style="color:var(--purple)">SKU ID</th><th style="color:var(--purple)">VIN</th>
            <th style="color:var(--purple)">Ent ID</th><th style="color:var(--purple)">Enterprise</th>
            <th onclick="_vwRepeatSort=_vwRepeatSort==='desc'?'asc':'desc';renderVinWise()" style="cursor:pointer;user-select:none;color:var(--purple)">Age <span id="vw-repeat-sort-icon">↓</span></th>
            <th style="color:var(--purple)">First QC Done</th>
          </tr></thead>
          <tbody id="vw-repeat-tbody"></tbody>
        </table>
      </div></div>
    </div>
  </div>
  <!-- VIN Status — assigned users -->
  <div style="margin-top:20px">
    <div style="font-size:13px;font-weight:700;color:var(--t1);margin-bottom:10px;display:flex;align-items:center;gap:8px">
      VIN Status
      <span style="font-size:10px;font-family:var(--f-mono);color:var(--t3);font-weight:400">assigned users · by user &amp; enterprise</span>
    </div>
    <div class="tbl-shell"><div class="tbl-scroll" style="max-height:175px">
      <table>
        <thead><tr>
          <th style="min-width:130px">QC User</th>
          <th style="min-width:150px">Enterprise</th>
          <th style="text-align:center;color:var(--green)">Below 6h</th>
          <th style="text-align:center;color:var(--red)">Above 6h</th>
          <th style="text-align:center;color:var(--purple)">Repeat VINs</th>
        </tr></thead>
        <tbody id="vw-user-tbody"></tbody>
      </table>
    </div></div>
  </div>
</div>
...
function f(){
  ...
  if(_abTab){
    const now=Date.now();
    const getH=r=>{const d=new Date(r.createdAt);return isNaN(d)?null:(now-d)/3600000;};
    if(_abTab==='repeat'){
      rows=rows.filter(r=>r.firstQcDone&&r.firstQcDone.trim()!=='');
    } else {
      const t=AB_TABS.find(t=>t.key===_abTab);
      if(t) rows=rows.filter(r=>{
        if(r.firstQcDone&&r.firstQcDone.trim()!=='')return false;
        const h=getH(r);return h!=null&&h>=t.min&&(t.max===9e9?true:h<t.max);
      });
    }
  }
...
function renderMetrics(rows){
  ...
  const procCount = _procRows.length;
  const items=[
    ...
    {icon:'⚙️',val:procCount, label:'Processing',     sub:'AI Pending',  click:true},
  ];
  document.getElementById('metrics').innerHTML=items.map(i=>
    `<div class="metric" ${i.click?'onclick="scrollToProcessing()" style="cursor:pointer" title="View Processing list"':''}>
     <div class="metric-icon">${i.icon}</div>
     <div class="metric-val" ${i.val>9999?'style="font-size:24px"':''}>${typeof i.val==='number'?i.val.toLocaleString():i.val}</div>
...
// ── Age breakdown — 4 clickable tab cards ─────────────────────────
let _abTab = null; // null=none selected

function setAbTab() {} // no-op

const AB_TABS = [
  { key: '0-4',    label: '0–4h',    color: 'var(--green)',  bg: 'rgba(0,200,150,.08)',  border: 'rgba(0,200,150,.3)',  min: 0,  max: 4  },
  { key: '4-6',    label: '4–6h',    color: 'var(--amber)',  bg: 'rgba(245,166,35,.08)', border: 'rgba(245,166,35,.3)', min: 4,  max: 6  },
  { key: 'over6',  label: 'Over 6h', color: 'var(--red)',    bg: 'rgba(240,81,110,.08)', border: 'rgba(240,81,110,.3)', min: 6,  max: 9e9 },
  { key: 'repeat', label: 'Repeat',  color: 'var(--purple)', bg: 'rgba(180,143,255,.08)',border: 'rgba(180,143,255,.3)',min: -1, max: -1  },
];

function renderHours(rows) {
  const now = Date.now();
  const getAge = r => { const d=new Date(r.createdAt); return isNaN(d)?null:(now-d)/3600000; };

  const nonRepeat = rows.filter(r => !r.firstQcDone || r.firstQcDone.trim() === '');
  const repeat    = rows.filter(r => r.firstQcDone && r.firstQcDone.trim() !== '');

  const counts = {
    '0-4':   nonRepeat.filter(r => { const h=getAge(r); return h!=null&&h>=0&&h<4; }).length,
    '4-6':   nonRepeat.filter(r => { const h=getAge(r); return h!=null&&h>=4&&h<6; }).length,
    'over6': nonRepeat.filter(r => { const h=getAge(r); return h!=null&&h>=6; }).length,
    'repeat': repeat.length,
  };

  const el = document.getElementById('hcards');
  el.innerHTML = AB_TABS.map(t => {
    const n = counts[t.key];
    const isActive = _abTab === t.key;
    return `<div onclick="clickAbTab('${t.key}')" style="cursor:pointer;background:${t.bg};border:${isActive?'2px':'1px'} solid ${t.border};border-radius:10px;padding:10px 14px;transition:all .15s;user-select:none;${isActive?'box-shadow:0 0 0 1px '+t.border:''}">
      <div style="font-size:9px;font-family:var(--f-mono);color:${t.color};letter-spacing:.8px;margin-bottom:4px">${t.label.toUpperCase()}</div>
      <div style="font-size:22px;font-weight:800;color:${t.color};letter-spacing:-1px;line-height:1">${n.toLocaleString()}</div>
      <div style="font-size:10px;color:${t.color};opacity:.6;margin-top:3px">${isActive ? '▼ filtered' : 'SKUs'}</div>
    </div>`;
  }).join('');

  document.getElementById('hr-tag').textContent = _abTab
    ? (AB_TABS.find(t=>t.k===_abTab)?.label || _abTab).toUpperCase()
    : 'CREATED AT';
}

function clickAbTab(key) {
  _abTab = _abTab === key ? null : key; // toggle
  // Apply as age/repeat filter and rerender
  _activeBkt = null; // clear old bucket filter
  rerender();
}

// ── Segment Status ─────────────────────────────────────────────────
function renderSegmentStatus(rows, tbodyId) {
  const now = Date.now();
  const getAge = r => { const d=new Date(r.createdAt); return isNaN(d)?null:(now-d)/3600000; };
  const segMap = {};
  rows.forEach(r => {
    const seg = r.customerSegment?.trim() || '(blank)';
    if (!segMap[seg]) segMap[seg] = { within: 0, over: 0, repeat: 0 };
    const isRepeat = r.firstQcDone && r.firstQcDone.trim() !== '';
    if (isRepeat) {
      segMap[seg].repeat++;
    } else {
      const h = getAge(r);
      if (h != null) { if (h < 6) segMap[seg].within++; else segMap[seg].over++; }
    }
  });
  const segs = Object.entries(segMap)
    .filter(([,v]) => v.within + v.over + v.repeat > 0)
    .sort((a,b) => (b[1].within+b[1].over+b[1].repeat) - (a[1].within+a[1].over+a[1].repeat));
  // Update tag if exists
  const tag = document.getElementById('seg-tag');
  if (tag) tag.textContent = segs.length + ' segments';
  const tbody = document.getElementById(tbodyId);
  if (!tbody) return;
  if (!segs.length) {
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;padding:16px;font-family:var(--f-mono);font-size:10px;color:var(--t3)">No data</td></tr>`;
    return;
  }
  tbody.innerHTML = segs.map(([seg, v], i) => `<tr style="border-top:1px solid var(--rim);${i%2===0?'':'background:var(--card2)'}">
    <td style="padding:7px 12px;font-size:11px;font-weight:500;color:var(--t1)">${seg}</td>
    <td style="padding:7px 12px;text-align:center;font-family:var(--f-mono);font-weight:700;color:${v.within>0?'var(--green)':'var(--t3)'}">${v.within}</td>
    <td style="padding:7px 12px;text-align:center;font-family:var(--f-mono);font-weight:700;color:${v.over>0?'var(--red)':'var(--t3)'}">${v.over}</td>
    <td style="padding:7px 12px;text-align:center;font-family:var(--f-mono);font-weight:700;color:${v.repeat>0?'var(--purple)':'var(--t3)'}">${v.repeat}</td>
  </tr>`).join('');
}
...
function renderCharts(rows){
  ...
  document.getElementById('ch-crm-sub').textContent = `${rows.length} SKUs`;
  // QC User table: group by qc_user + enterprise, split by age
  const now2 = Date.now();
  const getA = r => { const d = new Date(r.createdAt); return isNaN(d) ? null : (now2-d)/3600000; };
  const uMap = {};
  rows.filter(r => r.crmStatus !== 'qc_unassigned' && r.crmStatus !== 'qc_onhold').forEach(r => {
    const user = r.assignedTeam || '—';
    const ent  = r.entName || '—';
    const key  = user + '||' + ent;
    if (!uMap[key]) uMap[key] = { user, ent, below: 0, above: 0, repeat: 0 };
    const isRepeat = r.firstQcDone && r.firstQcDone.trim() !== '';
    if (isRepeat) {
      uMap[key].repeat++;
    } else {
      const h = getA(r);
      if (h != null) { if (h < 6) uMap[key].below++; else uMap[key].above++; }
    }
  });
  const uRows = Object.values(uMap).sort((a,b) => (b.below+b.above+b.repeat) - (a.below+a.above+a.repeat));
  const tbody = document.getElementById('ch-crm');
  if (!uRows.length) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:16px;font-family:var(--f-mono);font-size:10px;color:var(--t3)">No assigned users</td></tr>';
  } else {
    tbody.innerHTML = uRows.map((r,i) => `<tr style="border-top:1px solid var(--rim);${i%2===0?'':'background:var(--card2)'}">
      <td style="padding:7px 10px;font-size:11px;font-weight:500;color:var(--t1)">${r.user === '—' ? '<span style="color:var(--t3);font-style:italic;font-size:10px">(blank)</span>' : r.user}</td>
      <td style="padding:7px 10px;font-size:10px;color:var(--t2);max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${r.ent}">${r.ent}</td>
      <td style="padding:7px 10px;text-align:center;font-family:var(--f-mono);font-weight:600;color:var(--green)">${r.below}</td>
      <td style="padding:7px 10px;text-align:center;font-family:var(--f-mono);font-weight:600;color:${r.above>0?'var(--red)':'var(--t3)'}">${r.above}</td>
      <td style="padding:7px 10px;text-align:center;font-family:var(--f-mono);font-weight:600;color:${r.repeat>0?'var(--purple)':'var(--t3)'}">${r.repeat}</td>
    </tr>`).join('');
  }
  updateResetBtn();
}
...
function resetAllFilters(){_activeInputType=null;_activeBkt=null;_abTab=null;_activeCustomerSegment=null;_activeCrmStatus=null;syncTypeActive();rerender();}
function anyFilterActive(){return!!(_activeInputType||_activeBkt||_abTab||_activeCustomerSegment||_activeCrmStatus);}
...
// ── Auto-refresh ───────────────────────────────────────────────────
const AUTO_MS=15*60*1000;let _refreshStart=Date.now(),_refreshTimer=null;
...
function dlVwData(section, fmt) {
  ...
  const base = [..._rows].filter(r => r.crmStatus === 'qc_unassigned');
  let rows;
  if (section === 'repeat') {
    rows = base.filter(r => r.firstQcDone && r.firstQcDone.trim() !== '');
  } else {
    const nonRepeat = base.filter(r => !r.firstQcDone || r.firstQcDone.trim() === '');
    rows = nonRepeat.filter(r => {
      const h = getAge(r);
...
let _vwBelowSort = 'desc', _vwAboveSort = 'desc', _vwRepeatSort = 'desc', _vwCategory = null;
...
function renderVinWise() {
  ...
  let rows = [..._rows].filter(r => r.crmStatus === 'qc_unassigned');

  // Build category dropdown options (all unique segments)
  const cats = [...new Set(rows.map(r => r.customerSegment).filter(Boolean))].sort();
  const menu = document.getElementById('vw-cat-menu');
  const btn  = document.getElementById('vw-cat-btn');
  if (menu) {
    menu.innerHTML = `<button class="dl-item" onclick="_vwCategory=null;closeDl('vw-cat-dd');renderVinWise()" style="${!_vwCategory?'color:var(--teal)':''}">All</button>`
      + cats.map(c => `<button class="dl-item" onclick="_vwCategory='${c.replace(/'/g,"\\'")}';closeDl('vw-cat-dd');renderVinWise()" style="${_vwCategory===c?'color:var(--teal)':''}">${c}</button>`).join('');
  }
  if (btn) btn.firstChild.textContent = _vwCategory || 'All ';

  // Apply category filter
  if (_vwCategory) rows = rows.filter(r => r.customerSegment === _vwCategory);

  // Split: repeat = firstQcDone set; non-repeat = firstQcDone empty
  const repeat  = rows.filter(r => r.firstQcDone && r.firstQcDone.trim() !== '')
                      .sort((a,b) => _vwRepeatSort === 'desc' ? (getAge(b)||0) - (getAge(a)||0) : (getAge(a)||0) - (getAge(b)||0));
  const nonRepeat = rows.filter(r => !r.firstQcDone || r.firstQcDone.trim() === '');

  const below = nonRepeat.filter(r => { const h = getAge(r); return h != null && h < 6; })
                         .sort((a,b) => _vwBelowSort === 'desc' ? (getAge(b)||0) - (getAge(a)||0) : (getAge(a)||0) - (getAge(b)||0));
  const above = nonRepeat.filter(r => { const h = getAge(r); return h != null && h >= 6; })
  ...
  document.getElementById('vw-repeat-count').textContent = repeat.length.toLocaleString();
  ...
  const ri = document.getElementById('vw-repeat-sort-icon');
  if (ri) ri.textContent = _vwRepeatSort === 'desc' ? '↓' : '↑';

  // Row builder helper
  const mkRow = (r, i, color, extraCols) => {
    const h = getAge(r);
    const bg = i%2===0 ? '' : 'background:var(--card2)';
    return `<tr style="border-top:1px solid var(--rim);${bg}">
      <td data-copy="${r.sku||''}" onclick="copyCell(this.dataset.copy,this)" title="Click to copy SKU" style="padding:5px 6px;font-family:var(--f-mono);font-size:9px;color:var(--t2);cursor:copy;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(r.sku||'').substring(0,10)}…</td>
      <td data-copy="${r.vin||''}" onclick="copyCell(this.dataset.copy,this)" title="Click to copy VIN" style="padding:5px 6px;font-family:var(--f-mono);font-size:9px;color:var(--t1);font-weight:500;cursor:copy;white-space:nowrap">${r.vin||'—'}</td>
      <td data-copy="${r.eid||''}" onclick="copyCell(this.dataset.copy,this)" title="Click to copy Enterprise ID" style="padding:5px 6px;font-family:var(--f-mono);font-size:9px;color:var(--t2);cursor:copy;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(r.eid||'—').substring(0,8)}…</td>
      <td data-copy="${r.entName||''}" onclick="copyCell(this.dataset.copy,this)" title="Click to copy Enterprise" style="padding:5px 6px;font-family:var(--f-mono);font-size:9px;color:var(--t2);cursor:copy;max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.entName||'—'}</td>
      ${extraCols(r, h)}
    </tr>`;
  };
  ...
  bTbody.innerHTML = !below.length
    ? '<tr><td colspan="6" style="text-align:center;padding:20px;font-family:var(--f-mono);font-size:10px;color:var(--t3)">No VINs below 6h</td></tr>'
    : below.map((r,i) => {
        const h = getAge(r);
        const remaining = 6 - h;
        const isWarning = remaining <= 0.5;
        const slaEl = isWarning
          ? `<span style="font-size:9px;background:rgba(245,166,35,.15);color:var(--amber);border:1px solid rgba(245,166,35,.3);padding:2px 6px;border-radius:4px;font-family:var(--f-mono)">⚠ ${Math.round(remaining*60)}m</span>`
          : `<span style="font-size:9px;background:rgba(0,200,150,.1);color:var(--green);border:1px solid rgba(0,200,150,.2);padding:2px 6px;border-radius:4px;font-family:var(--f-mono)">✓ OK</span>`;
        return mkRow(r, i, 'green', (r,h) => `
          <td style="padding:6px 8px;color:${isWarning?'var(--amber)':'var(--t2)'};font-weight:${isWarning?'600':'400'};font-family:var(--f-mono);font-size:11px">${fmtAge(h)}</td>
          <td style="padding:6px 8px">${slaEl}</td>`);
      }).join('');

  // Above 6h
  const aTbody = document.getElementById('vw-above-tbody');
  aTbody.innerHTML = !above.length
    ? '<tr><td colspan="6" style="text-align:center;padding:20px;font-family:var(--f-mono);font-size:10px;color:var(--t3)">No SLA breaches 🎉</td></tr>'
    : above.map((r,i) => {
        const h = getAge(r);
        return mkRow(r, i, 'red', (r,h) => `
          <td style="padding:6px 8px;color:var(--red);font-weight:600;font-family:var(--f-mono);font-size:11px">${fmtAge(h)}</td>
          <td style="padding:6px 8px;color:var(--red);font-family:var(--f-mono);font-weight:600;font-size:11px">+${fmtAge(h-6)}</td>`);
      }).join('');

  // Repeat VIN
  const rTbody = document.getElementById('vw-repeat-tbody');
  rTbody.innerHTML = !repeat.length
    ? '<tr><td colspan="6" style="text-align:center;padding:20px;font-family:var(--f-mono);font-size:10px;color:var(--t3)">No repeat VINs</td></tr>'
    : repeat.map((r,i) => {
        const h = getAge(r);
        return mkRow(r, i, 'purple', (r,h) => `
          <td style="padding:6px 8px;color:var(--purple);font-family:var(--f-mono);font-size:11px">${fmtAge(h)}</td>
          <td style="padding:6px 8px;font-size:9px;color:var(--t2);font-family:var(--f-mono)">${r.firstQcDone||'—'}</td>`);
      }).join('');
  // VIN Status table — assigned users only (exclude qc_unassigned & qc_onhold)
  const uMap2 = {};
  [..._rows].filter(r => r.crmStatus !== 'qc_unassigned' && r.crmStatus !== 'qc_onhold').forEach(r => {
    const user = r.assignedTeam || '—';
    const ent  = r.entName || '—';
    const key  = user + '||' + ent;
    if (!uMap2[key]) uMap2[key] = { user, ent, below: 0, above: 0, repeat: 0 };
    const isRepeat = r.firstQcDone && r.firstQcDone.trim() !== '';
    if (isRepeat) {
      uMap2[key].repeat++;
    } else {
      const h = getAge(r);
      if (h != null) { if (h < 6) uMap2[key].below++; else uMap2[key].above++; }
    }
  });
  const uRows2 = Object.values(uMap2).sort((a,b) => (b.below+b.above+b.repeat) - (a.below+a.above+a.repeat));
  const uTbody = document.getElementById('vw-user-tbody');
  if (uTbody) {
    if (!uRows2.length) {
      uTbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:16px;font-family:var(--f-mono);font-size:10px;color:var(--t3)">No assigned users</td></tr>';
    } else {
      uTbody.innerHTML = uRows2.map((r,i) => `<tr style="border-top:1px solid var(--rim);${i%2===0?'':'background:var(--card2)'}">
        <td style="padding:7px 12px;font-size:11px;font-weight:500;color:var(--t1)">${r.user === '—' ? '<span style="color:var(--t3);font-style:italic;font-size:10px">(blank)</span>' : r.user}</td>
        <td style="padding:7px 12px;font-size:10px;color:var(--t2);max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${r.ent}">${r.ent}</td>
        <td style="padding:7px 12px;text-align:center;font-family:var(--f-mono);font-weight:700;color:var(--green)">${r.below}</td>
        <td style="padding:7px 12px;text-align:center;font-family:var(--f-mono);font-weight:700;color:${r.above>0?'var(--red)':'var(--t3)'}">${r.above}</td>
        <td style="padding:7px 12px;text-align:center;font-family:var(--f-mono);font-weight:700;color:${r.repeat>0?'var(--purple)':'var(--t3)'}">${r.repeat}</td>
      </tr>`).join('');
    }
  }
  // Segment Status
  renderSegmentStatus([..._rows].filter(r => r.crmStatus === 'qc_unassigned'), 'vw-seg-tbody');
}
...
function switchTab(tab) {
  ...
  if (isPendency) {
    ...
    if (_rows.length) {
      // Data already loaded — render instantly, then refresh in background if stale
      renderVinWise();
      startVwTimer();
      const staleMs = 3 * 60 * 1000;
      const isStale = !_lastSynced || (Date.now() - new Date(_lastSynced)) > staleMs;
      if (isStale && !_vwFetching) {
        _vwFetching = true;
        fetchFresh(false).then(() => renderVinWise()).finally(() => { _vwFetching = false; });
      }
    } else {
      // No data yet (hard refresh) — fetch first, then render
      if (!_vwFetching) {
        _vwFetching = true;
        fetchFresh(false).then(() => {
          renderVinWise();
...
// ── Processing data ────────────────────────────────────────────────
const PROC_API = '/api/360-processing';
let _procRows = [];

async function loadProcessing() {
  try {
    const r = await fetch(PROC_API);
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const j = await r.json();
    _procRows = (j.rows || []).map(r => ({
      sku:        r.sku || r.spin_sku_id || '',
      vin:        r.vin || r.vinName || '',
      eid:        r.eid || r.enterpriseId || '',
      entName:    r.entName || r.enterprise_name || '',
      inputType:  r.inputType || r.input_type || '',
      crmStatus:  r.crmStatus || r.crm_status || '',
      createdAt:  r.skuCreatedOn || r.createdAt || '',
    }));
    renderProcessing();
    renderMetrics(f()); // refresh count card
  } catch(e) {
    console.warn('Processing fetch failed:', e.message);
  }
}
...
function renderProcessing() {
  const now = Date.now();
  const getAge = r => { const d=new Date(r.createdAt); return isNaN(d)?null:(now-d)/3600000; };
  const fmtH = h => h==null ? '—' : Math.floor(h)+'h '+String(Math.round((h%1)*60)).padStart(2,'0')+'m';

  const tbody = document.getElementById('proc-tbody');
  const tag   = document.getElementById('proc-tag');
  if (tag) tag.textContent = `${_procRows.length.toLocaleString()} PROCESSING`;
  if (!tbody) return;
  if (!_procRows.length) {
    tbody.innerHTML = '<tr><td colspan="7" class="no-data">No processing SKUs</td></tr>';
    return;
  }
  const sorted = [..._procRows].sort((a,b) => (getAge(b)||0) - (getAge(a)||0));
  tbody.innerHTML = sorted.map((r,i) => {
    const h = getAge(r);
    const ageColor = h!=null&&h>6 ? 'var(--red)' : h!=null&&h>4 ? 'var(--amber)' : 'inherit';
    return `<tr>
      <td><span style="font-family:var(--f-mono);font-size:9px">${(r.sku||'').substring(0,14)}…</span></td>
      <td style="font-family:var(--f-mono);font-size:10px">${r.vin||'—'}</td>
      <td><span style="font-family:var(--f-mono);font-size:9px;color:var(--t2)">${(r.eid||'').substring(0,8)}…</span></td>
      <td style="font-size:11px">${r.entName||'—'}</td>
      <td>${r.inputType ? `<span class="itype-chip ${r.inputType.toLowerCase().includes('video')?'itype-video':'itype-image'}">${r.inputType}</span>` : '—'}</td>
      <td style="font-size:10px;color:var(--t2)">${r.crmStatus||'—'}</td>
      <td style="font-family:var(--f-mono);color:${ageColor};font-weight:${h!=null&&h>4?'600':'400'}">${fmtH(h)}</td>
    </tr>`;
  }).join('');
}

function dlProcCsv() {
  if (!_procRows.length) return;
  const now = Date.now();
  const getAge = r => { const d=new Date(r.createdAt); return isNaN(d)?null:(now-d)/3600000; };
  const hdrs = ['SKU ID','VIN','Ent ID','Enterprise','Input Type','CRM Status','Age (hrs)'];
  const csv = [hdrs.join(','), ...[..._procRows].map(r => {
    const h = getAge(r);
    return [r.sku,r.vin,r.eid,r.entName,r.inputType,r.crmStatus,h!=null?h.toFixed(2):''].map(v=>{
      const s=String(v??''); return /[,"]/.test(s)?`"${s.replace(/"/g,'""')}"`  :s;
    }).join(',');
  })].join('\n');
  dl(new Blob([csv],{type:'text/csv'}),'360-processing.csv');
}

function scrollToProcessing() {
  document.getElementById('processing-sec')?.scrollIntoView({behavior:'smooth',block:'start'});
}
